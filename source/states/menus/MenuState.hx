				...
				FlxFlicker.flicker(spr, 1, 0.06, false, false, function(flick:FlxFlicker)
				{
					switch(itemListing[curSelected]) {
						case 'Play':
							...
							else if (SaveData.progression <= 2) {
								SideState.lvlId = 'street';
								Main.switchState(new SideState());
							}
							else if (SaveData.progression == 3) {
						...
						case 'Credits':
							...
							else if (logged == 312 && SaveData.data.get('Video Cutscenes')) {
								FlxG.sound.music.stop();
								Main.switchState(new VideoState('kuze', new Credits()));
							}
							else if (logged == 121 && SaveData.data.get('Video Cutscenes')) {
								FlxG.sound.music.stop();
								Main.switchState(new VideoState('abi', new Credits()));
							}
							else if (logged == 413 && SaveData.data.get('Video Cutscenes')) {
								FlxG.sound.music.stop();
								Main.switchState(new VideoState('FUCK', new Credits()));
							}
							else if (logged == 167 && SaveData.data.get('Video Cutscenes')) {
								FlxG.sound.music.stop();
